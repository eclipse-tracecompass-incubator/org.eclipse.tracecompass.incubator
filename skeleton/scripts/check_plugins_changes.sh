...
#
# Usage  ./check_plugins_changes.sh baselineBranch [ignoredCommit1,ignoredCommit2,...]
#
...
#
# For example ./check_plugins_changes.sh stable-1.0 1325468,1325469

if [ -z "$1" ]; then
	echo "usage: $0 baselineBranch [ignoredCommit1,ignoredCommit2,...]"
	exit 1
fi

IFS=', ' read -r -a IGNORED_COMMITS <<< "$2"

PREV_RELEASE_VERSION=$1
echo "Baseline branch entered: $PREV_RELEASE_VERSION. If this is wrong, stop the script (Ctrl-C)"
...
for plugin_path in "${ALL_PLUGIN_PATHS[@]}"; do
	...
	manifest_diff=$(git diff $PREV_RELEASE_VERSION -- $plugin_path/META-INF/MANIFEST.MF)
	...
	plugin_diff=$(git diff $PREV_RELEASE_VERSION $commit_id -- "$plugin_path")
	...
	let next_micro+=1
	git diff $PREV_RELEASE_VERSION $commit_id -- "$plugin_path"
