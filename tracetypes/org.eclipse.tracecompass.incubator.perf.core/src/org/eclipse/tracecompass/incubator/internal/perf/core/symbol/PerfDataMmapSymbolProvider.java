...
public class PerfDataMmapSymbolProvider implements ISymbolProvider {
     ...
     */
    public PerfDataMmapSymbolProvider(@NonNull ITmfTrace trace, PerfDataMmapAnalysisModule module) {
        fTrace = trace;
        fMmapModule = module;
        module.schedule();
    }
    ...
    public @Nullable TmfResolvedSymbol getSymbol(int pid, long timestamp, long address) {
        ...
        try {
            Iterable<@NonNull ITmfStateInterval> intervals = ss.query2D(baddrQuarks, Collections.singleton(timestamp));
    ...
    private @Nullable TmfResolvedSymbol resolveInFile(int pid, @NonNull String filename, long address, long loadBase) {
