...
import org.eclipse.tracecompass.incubator.internal.jifa.core.gclog.diagnoser.EventAbnormalSet;
import org.eclipse.tracecompass.incubator.internal.jifa.core.gclog.diagnoser.EventDiagnoseInfo;
...
public class TestEventDiagnoser {
    ...
    public void testDiagnoseBasic() throws Exception {
        ...
        for (GCEvent event : model.getAllEvents()) {
            ...
            if (type == GCEventType.YOUNG_GC) {
                hit++;
                absContainExactTypes(abs, List.of(AbnormalType.TO_SPACE_EXHAUSTED, AbnormalType.BAD_DURATION, AbnormalType.BAD_PROMOTION));

            } else if (type == GCEventType.G1_EVACUATION_FAILURE) {
                hit++;
                absContainExactTypes(abs, List.of(AbnormalType.BAD_EVENT_TYPE, AbnormalType.BAD_DURATION));
            } else {
                assert abs.isEmpty();
            }
        }
    ...
    private static void absContainExactTypes(EventAbnormalSet abs, List<AbnormalType> types) {
