...
import static org.eclipse.tracecompass.incubator.internal.jifa.core.gclog.diagnoser.AnalysisConfig.defaultConfig;
...
import org.eclipse.tracecompass.incubator.internal.jifa.core.gclog.diagnoser.AbnormalType;
...
import org.eclipse.tracecompass.incubator.internal.jifa.core.gclog.diagnoser.GlobalDiagnoser.GlobalAbnormalInfo;
import org.eclipse.tracecompass.incubator.internal.jifa.core.gclog.diagnoser.GlobalDiagnoser.MostSeriousProblemSummary;
...
public class TestGlobalDiagnoser {
    ...
    public void testDiagnoseBasic() throws Exception {
        ...
        List<Double> list = seriousProblems.get(AbnormalType.METASPACE_FULL_GC.getName());
        assertNotNull(list);
        Assert.assertArrayEquals(list.stream().mapToDouble(d -> d).toArray(),
                new double[]{3765, 12765, 103765, 120765, 155765}, DELTA);
        Assert.assertArrayEquals(seriousProblems.getOrDefault(AbnormalType.SYSTEM_GC.getName(), Collections.emptyList()).stream().mapToDouble(d -> d).toArray(),
                new double[]{80765}, DELTA);
        Assert.assertArrayEquals(seriousProblems.getOrDefault(AbnormalType.HEAP_MEMORY_FULL_GC.getName(), Collections.emptyList()).stream().mapToDouble(d -> d).toArray(),
        ...
        List<Double> list2 = seriousProblems.get(AbnormalType.METASPACE_FULL_GC.getName());
        assertNotNull(list2);
    ...
    public void testDiagnoseZGC() throws Exception {
        ...
        Assert.assertArrayEquals(seriousProblems.getOrDefault(AbnormalType.ALLOCATION_STALL.getName(), Collections.emptyList()).stream().mapToDouble(d -> d).toArray(),
                new double[]{7777 - 0.204, 7888 - 0.391}, DELTA);
        Assert.assertArrayEquals(seriousProblems.getOrDefault(AbnormalType.OUT_OF_MEMORY.getName(), Collections.emptyList()).stream().mapToDouble(d -> d).toArray(),
    ...
    private static double[] sitesToArray(List<TimeRange> sites) {
