...
package org.eclipse.tracecompass.incubator.internal.syslog.core.gclog.util;

import static org.eclipse.tracecompass.incubator.internal.syslog.core.gclog.util.Constant.MS2S;
...
public class GCLogUtil {
     ...
     * e.g. "10m" -> 10485760, "111 KB" -> 113664 do not check format, do not
     * consider b(bit)
     */
    public static long toByte(String sizeString, long divideIfNoUnit) {
        String sizeStringFixed = sizeString.toLowerCase();
        int mid;
        for (mid = 0; mid < sizeStringFixed.length(); mid++) {
            char c = sizeStringFixed.charAt(mid);
    ...
    /**
     * find the last time and parse it into time in ms e.g. "123ms" -> 123,
     * "12s" -> 12000 do not check format
     */
    ...
    /**
     * e.g. "user=0.15s sys=0.01s real=0.02s","user=0.04 sys=0.00, real=0.01
     * secs"
     */
    ...
    /**
     * e.g. "Concurrent Clear Claimed Marks 0.009ms", "Concurrent Clear Claimed
     * Marks" -> "0.009ms" "Pre Evacuate Collection Set: 0.0ms" , "Pre Evacuate
     * Collection Set" -> "0.0ms" do not check format
     */
     ...
     * "3604K(262144K)->3608K(262144K)" -> ["3604K", "262144K", "3608K",
     * "262144K"] "608K(262144K)" -> [null, null, "3608", "262144"] do not check
     * format
     */
    ...
    /**
     * e.g. "3604K->3608K(262144K)" -> [3604, -1, 3608, 262144] "3604K->3608K"
     * -> [3604, -1, 3608, -1] "3604K(262144K)->3608K(262144K)" -> [3604, -1,
     * 3608, 262144] "608K(262144K)" -> [-1, -1, 3608, 262144] do not check
     * format
     */
    ...
    // e.g " Pause Young (Normal) (G1 Evacuation Pause) " -> ["Pause", "Young",
    ...
    public static long parseDateStamp(String text) {
        ...
        String textFixed = text.substring(0, text.length() - 2) + ":" + text.substring(text.length() - 2);
        OffsetDateTime odt = OffsetDateTime.parse(textFixed);
        return odt.toInstant().toEpochMilli();
    }

    // check text is a decimal from index and there are digitNumberAfterDot
    // after dot
